...
def run_mmp(
    ...
    if prev_actions is not None:
        # print(policy,policy.shape, prev_actions, prev_actions.shape, type(prev_actions))
        ...
        for t in range(infer_len):
            for f in range(num_factors):
                ...
                if t == 0:
                    ...
                    past_msg = B[f][:, :, int(policy[t - 1])].dot(qs_seq[t - 1][f])
                ...
                if t >= future_cutoff:
                    ...
                    future_msg = trans_B[f][:, :, int(policy[t])].dot(qs_seq[t + 1][f])
