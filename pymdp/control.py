...
def update_posterior_policies_full(
    ...
    print("Update posterior policies Before loop Duration:", elend-elstart)
    elstart = datetime.datetime.now()
    #NOTE: Loop to vectorize
    ...
    elend = datetime.datetime.now()
    print("Update posterior policies  loop Duration:", elend-elstart)
...
def get_expected_states(qs, B, policy):
    ...
    # n_factors = policy.shape[1] #NOTE: Changed
    n_factors = B.shape[0]
    ...
    # print(len(qs_pi),len(qs),n_factors, n_steps, policy)
    # get expected states over time
    for t in range(n_steps):
        for control_factor in range(n_factors): #NOTE: Added
            action = policy[t]
            # if qs_pi[t+1][control_factor] is  None:
            #     print("GGG",B[control_factor][:,:,int(action)].dot(qs_pi[t][control_factor]).shape)
            #     print("^^^^",  qs_pi[t+1][control_factor], qs_pi[t+1][control_factor].shape)
            # print(type(qs_pi[t][control_factor]))
            qs_pi[t+1][control_factor] = B[control_factor][:,:,int(action)].dot(qs_pi[t][control_factor])
            
        # for control_factor, action in enumerate(policy[t,:]): #NOTE: original
        #     qs_pi[t+1][control_factor] = B[control_factor][:,:,int(action)].dot(qs_pi[t][control_factor])#NOTE: original
...
def construct_policies(num_states, num_controls = None, policy_len=1, control_fac_idx=None):
    ...
    for pol_i in range(len(policies)):
        ...
        policies[pol_i] = np.array(policies[pol_i]).reshape(policy_len,1)
        
    # print(policies)
    print("Numebr of policies: ",len(policies))
    # exit()
    return policies
...
def sample_action(q_pi, policies, num_controls, action_selection="deterministic", alpha = 16.0):
    ...
    for pol_idx, policy in enumerate(policies):
        ...
        # for factor_i, action_i in enumerate(policy[0, :]): #NOTE: Removed
            action_marginals[factor_i][policy[0]] += q_pi[pol_idx]
            # action_marginals[factor_i][action_i] += q_pi[pol_idx] #NOTE: Removed
